...
@section('content')
    <main class="px-20 py-40">
        ...
        <!-- CSV Upload Form -->
        <form action="{{ route('quiz.parse') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
            ...
            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md">Upload CSV</button>
        </form>

        {{-- Load  Variable passed from controller text if exists --}}
        @if (isset($text))
        ...
        <form id="quiz-form" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4 flex space-x-4">
                <div>
                    ...
                    <input type="date" name="quiz_date" id="quiz_date"
                        class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required>
                </div>
                <div>
            ...
            <button type="button" id="add-question" class="bg-blue-500 text-white px-4 py-2 rounded-md mb-4">Add
                Question</button>

            <!-- Submit Button -->
            <button type="submit" id="submit-button" class="mt-4 bg-green-500 text-white px-6 py-3 rounded-md">Create
                Quiz</button>
        </form>
    </main>
    ...
    <script>
                ...
                reader.onload = function(e) {
                    ...
                    lines.forEach((line) => {
                        if (line.trim().length > 0) {
                            // Properly split the line with regex to handle commas inside quoted parts
                            const regex = /,(?=(?:(?:[^"]*"){2})*[^"]*$)/;
                            const parts = line.split(regex);

                            const type = parts[0]?.trim();
                            const question = parts[1]?.trim();
                            const rawChoices = parts[2]?.trim() || '';
                            const answer = parts[3]?.trim();

                            const choices = rawChoices.split(';').map(choice => choice
                                .trim()); // Now splitting by semicolon
                            const parsedQuestionData = {
                                type: type,
                                question: question,
                                choices: choices,
                                answer: answer.replace(/^"|"$/g,
                                    '') // Trim surrounding quotes for answers
                            };
                            createQuestionElement(questionIndex, parsedQuestionData);
                        }
                    });
                };
        ...
        function createQuestionElement(index, questionData = {}) {
            ...
            <div class="flex items-center mb-2">
                ...
                <input type="radio" name="questions[${index}][correct]" value="${choiceIndex}" class="ml-2" ${choice === questionData.answer ? 'checked' : ''}> Correct
        ...
        document.getElementById('quiz-form').addEventListener('submit', function(event) {
            ...
            questionElements.forEach((element, i) => {
                ...
                if (questionType === 'multiple_choice') {
                    const choices = Array.from(element.querySelectorAll(
                            `input[name="questions[${i}][choices][]"]`))
                        .map(choiceInput => choiceInput.value);

                    const correctIndex = Array.from(element.querySelectorAll(
                            `input[name="questions[${i}][correct]"]`))
                        .findIndex(radio => radio.checked);

                    questionData.choices = choices;
                    questionData.answer = correctIndex !== -1 ? choices[correctIndex] : '';
                } else {
                    questionData.answer = element.querySelector(`input[name="questions[${i}][answer]"]`)
                        .value ||
