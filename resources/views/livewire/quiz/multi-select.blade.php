<section class="bg-white flex-1 p-10 shadow-md rounded-lg relative">
    ...
    @foreach ($question['questionChoices'] as $choice)
        <div wire:key="{{ $choice->id }}" id="answer-box" class="mt-6 space-y-4 text-sm">
            <div class="flex items-center gap-4">
                <input type="checkbox" wire:model.change="answers" name="answer" id="answer-{{ $choice->id }}"
                    value="{{ $choice->id }}"
                    class="answer-checkbox h-5 w-5 text-blue-500 focus:ring-blue-400 border-gray-300">
                <label for="answer-{{ $choice->id }}" class="text-gray-700 cursor-pointer">
                    {{ $choice['content'] }}
                </label>
            </div>
        </div>
