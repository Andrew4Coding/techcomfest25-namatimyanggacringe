<a href="{{ url('/courses/' . $course->id) }}" class="no-underline text-black h-full">
    <div class="w-full relative overflow-hidden shadow-lg h-full rounded-xl hover:scale-[102%] duration-300 course-{{ $course->theme }}"
        style="background-color: {{ $theme['primary'] }}; color: {{ $theme['secondary'] ?? $theme['secondary'] }}">
        @if ($course->theme == 'sosiologi')
